...
- **2026/08/07**: 📄 **新论文：[YOLO-PEFT：YOLO 系列的参数高效微调](https://arxiv.org/abs/2608.07051)（[PDF](https://arxiv.org/pdf/2608.07051)）** — 提出面向 YOLO 检测器 PEFT 放置的结构感知、可审计约束规划框架。在论文覆盖的评测范围内，规划器选择的 RS-LoRA 在 YOLO11s / YOLO12s 上达到 **0.7138 / 0.7307 mAP50-95**，高于 Full-SFT 的 **0.6428 / 0.6662**；在 RT-DETR-L 上评测的 7 种 LoRA 系列配置均触发校准后的 Refuse-to-Full-SFT 决策。受控 YOLO11 审计显示，LoRA 可将训练峰值显存降低 **43.9%**，但训练时间为 **1.72 倍**。冻结声明表：[`reports/yolo-peft-2608.07051-anchor.json`](reports/yolo-peft-2608.07051-anchor.json)；用 `python scripts/reproduce_yolo_peft_paper.py --check-anchor` 校验。
